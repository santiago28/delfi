...
class HomeController extends ControllerBase
{
    ...
    public function indexhomeAction()
    {
		...
		$contratos_arr = ContratoXSede::find( array( " id_modalidad IN (1,5,6,7,8,11,12,13) ", "group"=>"id_contrato" , "order" => "id_contrato" ) );
		...
		$bloq_cont_mes[$j][] = "$dist_contratos[$j]"." - "."$dist_contratos_modalidad[$j]"." : "."$dist_contratos_prestador[$j]";
			for ($i=1; $i <= 13 ;$i++){
				...
				if (count($consulta) == 0) {
					...
					//}
					$bloq_cont_mes[$j][] = count($consulta);
						//$bloq_cont_mes[$j][] = $componentesNuevo;
				}
			}

			$sql = "SELECT ContratoXInterventor.id_interventor FROM ContratoXInterventor WHERE ContratoXInterventor.id_contrato = '".$dist_contratos[$j]."' AND ContratoXInterventor.estado = '1'";
			$query = $this->modelsManager->createQuery($sql);
			$interventors = $query->execute();
			foreach ($interventors as $interventor) {
			    //echo "Name: ", $interventor->id_interventor, "\n";
			    $bloq_cont_mes[$j][] = $interventor->id_interventor;
			}
			//$interventor = ContratoXInterventor::find(	array(	"id_contrato='$dist_contratos[$j]' AND estado= '1' " ));
			//$bloq_cont_mes[$j][] = print_r($interventor);
		}
		...
		if($id_group==1){	//Administradores
			$listacontratos = ContratoXSede::find(	array(	" id_modalidad IN (1,5,6,7,8,11,12,13) AND estado= '1' ",	"group"=> "id_contrato" ));
		...
		//Query para el select de los meses para ajuste - DANIEL GALLO
		$mesesAjustes = Mes::find(	array(	" (estado= '0' AND bloqueo_total='1') OR (estado= '1' AND bloqueo_total='1') " ));
		if (count($mesesAjustes) == 0) {
			//$this->flash->error("Mensaje de Error");
			$mesesAjustes = null;
		}
		...
		$this->view->mesesAjustes = $mesesAjustes;
	    ...
	    public function indexdetallebloqueoAction($variable)
    {
		...
		$bloq_cont_mes = array();
			...
			foreach($bloqueados as $x){
				$j = 0;
				switch ($x->nombre_componente_bloqueado) {
				 	case 'CANASTA':
				 		$j = 0;
						break;
				 	case 'COSTOS':
				...
				if ($bloq_cont_mes[$j][0] != $x->nombre_componente_bloqueado){
					$bloq_cont_mes[$j][0] = $x->nombre_componente_bloqueado;
				}
				$bloq_cont_mes[$j][$x->id_mes] = "X";
			}
		$this->view->contrato = $variable;
		$this->view->bloq_cont_mes = $bloq_cont_mes;
		$this->view->bloqueados = $bloqueados;
		// $this->assets
		// ->addJs('js/filtros-tablesorter.min.js');
